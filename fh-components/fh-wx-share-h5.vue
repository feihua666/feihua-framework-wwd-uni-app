    ...
    export default {
        ...
        methods:{
            myready (shareContent) {
                ...
                let singlemessageLink = self.$utils.concatUrlParam(link,{from: 'singlemessage',f_u: self.$store.state.userinfo.id})
                let timelineLink = self.$utils.concatUrlParam(link,{from: 'timeline',f_u: self.$store.state.userinfo.id})
