		...
		methods:{
            ...
            doAppendCopyInfo(){
                ...
                self.$http.getDictByValue('has_hourse_status',self.wwdUser.wwdUserDto.hasHourse).then(function (dict) {
                    let hourseCity = ''
                    if (self.wwdUser.wwdUserDto.hourseCity) {
                        hourseCity = '('+ self.wwdUser.wwdUserDto.hourseCity +')'
                    }
                    self.copyMyinfo.push('房：' + dict.name + (hourseCity))
                })
                self.$http.getDictByValue('has_car_status',self.wwdUser.wwdUserDto.hasCar).then(function (dict) {
                    let carCity = ''
					if (self.wwdUser.wwdUserDto.carCity) {
					    carCity = '('+ self.wwdUser.wwdUserDto.carCity +')'
					}
                    self.copyMyinfo.push('车：' + dict.name + (carCity))
                })
